<script setup lang="ts">
import { ref, computed, onMounted } from "vue";
import { apiClient } from "../../utils/api";
import { useToast } from "../../composables/useToast";
import Card from "../../components/ui/Card.vue";
import Button from "../../components/ui/Button.vue";
import Input from "../../components/ui/Input.vue";
import Badge from "../../components/ui/Badge.vue";
import Skeleton from "../../components/ui/Skeleton.vue";
import EmptyState from "../../components/ui/EmptyState.vue";
import Alert from "../../components/ui/Alert.vue";
import type { AdminPatientItem, PaginatedPatientResponse } from "@omnimedix/shared";

interface ApiResponse<T> {
  success: boolean;
  data: T;
  pagination?: {
    page: number;
    limit: number;
    total: number;
    totalPages: number;
    hasNextPage: boolean;
    hasPrevPage: boolean;
  };
  message?: string;
  error?: {
    code: string;
    message: string;
    details?: unknown;
  };
}

const toast = useToast();

// State Pasien
const patients = ref<AdminPatientItem[]>([]);
const pagination = ref({
  page: 1,
  limit: 10,
  total: 0,
  totalPages: 1,
  hasNextPage: false,
  hasPrevPage: false,
});
const isLoading = ref(true);
const isRefreshing = ref(false);
const errorMessage = ref<string | null>(null);

// State Filter & Search
const searchQuery = ref("");
const selectedActiveStatus = ref<"all" | "active" | "inactive">("all");

// Modal Tambah / Edit Pasien
const isFormModalOpen = ref(false);
const isEditing = ref(false);
const editingPatientId = ref<string | null>(null);
const isSubmitting = ref(false);
const formError = ref<string | null>(null);

// Form Data
const formData = ref({
  fullName: "",
  dateOfBirth: "",
  gender: "MALE" as "MALE" | "FEMALE",
  phone: "",
  nik: "",
  isActive: true,
});

// Form Errors
const formFieldErrors = ref<Record<string, string>>({});

// Modal Konfirmasi Nonaktifkan Pasien
const isDeactivateModalOpen = ref(false);
const patientToDeactivate = ref<AdminPatientItem | null>(null);
const isDeactivating = ref(false);

// Hitung usia dari tanggal lahir
function calculateAge(dobString: string): number {
  try {
    const birthDate = new Date(dobString);
    const today = new Date();
    let age = today.getFullYear() - birthDate.getFullYear();
    const m = today.getMonth() - birthDate.getMonth();
    if (m < 0 || (m === 0 && today.getDate() < birthDate.getDate())) {
      age--;
    }
    return Math.max(0, age);
  } catch {
    return 0;
  }
}

// Format tanggal standar Indonesia
function formatDate(dateString: string): string {
  try {
    const d = new Date(dateString);
    return new Intl.DateTimeFormat("id-ID", {
      day: "numeric",
      month: "short",
      year: "numeric",
    }).format(d);
  } catch {
    return dateString;
  }
}

// Fetch data pasien dari backend
async function fetchPatients(page = 1) {
  if (patients.value.length === 0) {
    isLoading.value = true;
  } else {
    isRefreshing.value = true;
  }
  errorMessage.value = null;

  try {
    const params = new URLSearchParams();
    params.set("page", String(page));
    params.set("limit", "10");

    if (searchQuery.value.trim()) {
      params.set("search", searchQuery.value.trim());
    }

    if (selectedActiveStatus.value === "active") {
      params.set("isActive", "true");
    } else if (selectedActiveStatus.value === "inactive") {
      params.set("isActive", "false");
    }

    const res = await apiClient<ApiResponse<AdminPatientItem[]>>(
      `/admin/patients?${params.toString()}`,
    );

    if (res.success && res.data) {
      patients.value = res.data;
      if (res.pagination) {
        pagination.value = res.pagination;
      }
    } else {
      throw new Error("Gagal menerima daftar pasien.");
    }
  } catch (err: unknown) {
    const errorObj = err as { data?: { error?: { message?: string } }; message?: string };
    errorMessage.value =
      errorObj.data?.error?.message ||
      errorObj.message ||
      "Gagal memuat daftar pasien. Silakan coba lagi.";
  } finally {
    isLoading.value = false;
    isRefreshing.value = false;
  }
}

// Handler Search & Filter
let searchDebounceTimer: ReturnType<typeof setTimeout> | null = null;
function onSearchInput() {
  if (searchDebounceTimer) clearTimeout(searchDebounceTimer);
  searchDebounceTimer = setTimeout(() => {
    fetchPatients(1);
  }, 350);
}

function onFilterChange() {
  fetchPatients(1);
}

// Buka Modal Tambah Pasien Baru
function openAddModal() {
  isEditing.value = false;
  editingPatientId.value = null;
  formError.value = null;
  formFieldErrors.value = {};
  formData.value = {
    fullName: "",
    dateOfBirth: "",
    gender: "MALE",
    phone: "",
    nik: "",
    isActive: true,
  };
  isFormModalOpen.value = true;
}

// Buka Modal Edit Pasien
function openEditModal(patient: AdminPatientItem) {
  isEditing.value = true;
  editingPatientId.value = patient.id;
  formError.value = null;
  formFieldErrors.value = {};
  formData.value = {
    fullName: patient.fullName,
    dateOfBirth: patient.dateOfBirth,
    gender: patient.gender,
    phone: patient.phone || "",
    nik: patient.nik || "",
    isActive: patient.isActive,
  };
  isFormModalOpen.value = true;
}

// Tutup Modal Form
function closeFormModal() {
  if (isSubmitting.value) return;
  isFormModalOpen.value = false;
  formError.value = null;
  formFieldErrors.value = {};
}

// Validasi Form di Frontend
function validateForm(): boolean {
  formFieldErrors.value = {};
  let valid = true;

  if (!formData.value.fullName.trim()) {
    formFieldErrors.value["fullName"] = "Nama lengkap pasien wajib diisi.";
    valid = false;
  } else if (formData.value.fullName.trim().length < 2) {
    formFieldErrors.value["fullName"] = "Nama minimal 2 karakter.";
    valid = false;
  }

  if (!formData.value.dateOfBirth) {
    formFieldErrors.value["dateOfBirth"] = "Tanggal lahir wajib diisi.";
    valid = false;
  } else {
    const dob = new Date(formData.value.dateOfBirth);
    if (dob > new Date()) {
      formFieldErrors.value["dateOfBirth"] = "Tanggal lahir tidak boleh di masa depan.";
      valid = false;
    }
  }

  if (formData.value.nik && formData.value.nik.trim()) {
    if (!/^\d{16}$/.test(formData.value.nik.trim())) {
      formFieldErrors.value["nik"] = "NIK harus terdiri dari 16 digit angka.";
      valid = false;
    }
  }

  if (formData.value.phone && formData.value.phone.trim()) {
    if (formData.value.phone.trim().length < 8) {
      formFieldErrors.value["phone"] = "Nomor telepon minimal 8 digit.";
      valid = false;
    }
  }

  return valid;
}

// Submit Form Tambah/Edit Pasien
async function handleSubmitForm() {
  if (!validateForm()) return;

  isSubmitting.value = true;
  formError.value = null;

  try {
    const payload = {
      fullName: formData.value.fullName.trim(),
      dateOfBirth: formData.value.dateOfBirth,
      gender: formData.value.gender,
      phone: formData.value.phone.trim() || undefined,
      nik: formData.value.nik.trim() || undefined,
      isActive: formData.value.isActive,
    };

    if (isEditing.value && editingPatientId.value) {
      // Update Pasien
      const res = await apiClient<ApiResponse<AdminPatientItem>>(
        `/admin/patients/${editingPatientId.value}`,
        {
          method: "PUT",
          body: payload,
        },
      );

      if (res.success && res.data) {
        toast.success(`Data pasien '${res.data.fullName}' berhasil diperbarui.`);
        isFormModalOpen.value = false;
        await fetchPatients(pagination.value.page);
      }
    } else {
      // Tambah Pasien Baru
      const res = await apiClient<ApiResponse<AdminPatientItem>>(
        "/admin/patients",
        {
          method: "POST",
          body: payload,
        },
      );

      if (res.success && res.data) {
        toast.success(`Pasien baru '${res.data.fullName}' berhasil didaftarkan.`);
        isFormModalOpen.value = false;
        await fetchPatients(1);
      }
    }
  } catch (err: unknown) {
    const errorObj = err as { data?: { error?: { message?: string } }; message?: string };
    formError.value =
      errorObj.data?.error?.message ||
      errorObj.message ||
      "Terjadi kesalahan saat menyimpan data pasien.";
  } finally {
    isSubmitting.value = false;
  }
}

// Buka Modal Konfirmasi Nonaktifkan
function openDeactivateModal(patient: AdminPatientItem) {
  patientToDeactivate.value = patient;
  isDeactivateModalOpen.value = true;
}

// Tutup Modal Konfirmasi Nonaktifkan
function closeDeactivateModal() {
  if (isDeactivating.value) return;
  isDeactivateModalOpen.value = false;
  patientToDeactivate.value = null;
}

// Eksekusi Nonaktifkan Pasien (Soft Delete)
async function handleConfirmDeactivate() {
  if (!patientToDeactivate.value) return;

  isDeactivating.value = true;

  try {
    const res = await apiClient<ApiResponse<AdminPatientItem>>(
      `/admin/patients/${patientToDeactivate.value.id}`,
      {
        method: "DELETE",
      },
    );

    if (res.success) {
      toast.info(`Pasien '${patientToDeactivate.value.fullName}' telah dinonaktifkan.`);
      closeDeactivateModal();
      await fetchPatients(pagination.value.page);
    }
  } catch (err: unknown) {
    const errorObj = err as { data?: { error?: { message?: string } }; message?: string };
    toast.error(
      errorObj.data?.error?.message ||
        errorObj.message ||
        "Gagal menonaktifkan data pasien.",
    );
  } finally {
    isDeactivating.value = false;
  }
}

onMounted(() => {
  fetchPatients(1);
});
</script>

<template>
  <div class="space-y-6">
    <!-- Top Header -->
    <div
      class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-800/80 pb-5"
    >
      <div>
        <div class="flex items-center gap-2 mb-1">
          <Badge variant="primary" size="sm" dot>Administrator</Badge>
          <span class="text-xs text-slate-500 font-mono tracking-wider uppercase">Master Rekam Medis</span>
        </div>
        <h1 class="text-2xl sm:text-3xl font-bold tracking-tight text-slate-100">
          Manajemen Data Pasien
        </h1>
        <p class="text-xs sm:text-sm text-slate-400 mt-1 max-w-2xl leading-relaxed">
          Kelola data kependudukan, kontak, dan riwayat status keaktifan pasien di fasilitas kesehatan.
        </p>
      </div>

      <div class="flex items-center gap-3">
        <Button
          variant="primary"
          size="sm"
          @click="openAddModal"
          class="shadow-sm shadow-blue-500/20"
        >
          <svg
            class="h-4 w-4 sm:mr-1.5"
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
            stroke-width="2"
          >
            <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
          </svg>
          <span class="font-medium">Tambah Pasien Baru</span>
        </Button>
      </div>
    </div>

    <!-- Filter & Search Toolbar -->
    <Card class="relative border-slate-800 bg-slate-900/70 backdrop-blur-sm p-4">
      <div class="absolute -top-1 -left-1 font-mono text-[9px] text-slate-700/60 select-none pointer-events-none">+</div>
      <div class="absolute -top-1 -right-1 font-mono text-[9px] text-slate-700/60 select-none pointer-events-none">+</div>
      <div class="absolute -bottom-1 -left-1 font-mono text-[9px] text-slate-700/60 select-none pointer-events-none">+</div>
      <div class="absolute -bottom-1 -right-1 font-mono text-[9px] text-slate-700/60 select-none pointer-events-none">+</div>

      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <!-- Search Input -->
        <div class="relative flex-1 max-w-md">
          <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-slate-500">
            <svg
              class="h-4 w-4"
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              stroke-width="2"
            >
              <path
                stroke-linecap="round"
                stroke-linejoin="round"
                d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z"
              />
            </svg>
          </div>
          <input
            v-model="searchQuery"
            type="text"
            placeholder="Cari nama pasien, NIK (16 digit), atau telepon..."
            @input="onSearchInput"
            class="w-full pl-9 pr-4 py-2 bg-slate-950 border border-slate-800 rounded-lg text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:border-blue-500 transition-colors"
          />
        </div>

        <!-- Filter Status Keaktifan -->
        <div class="flex items-center gap-2 shrink-0">
          <span class="text-xs text-slate-400 font-medium hidden sm:inline">Status:</span>
          <select
            v-model="selectedActiveStatus"
            @change="onFilterChange"
            class="bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-xs text-slate-200 focus:outline-none focus:border-blue-500 transition-colors cursor-pointer"
          >
            <option value="all">Semua Pasien</option>
            <option value="active">Hanya Pasien Aktif</option>
            <option value="inactive">Pasien Non-aktif</option>
          </select>

          <Button
            variant="outline"
            size="sm"
            :disabled="isLoading || isRefreshing"
            @click="fetchPatients(pagination.page)"
            title="Muat ulang tabel"
            class="px-2.5"
          >
            <svg
              :class="['h-3.5 w-3.5', (isLoading || isRefreshing) && 'animate-spin']"
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
            >
              <path
                stroke-linecap="round"
                stroke-linejoin="round"
                stroke-width="2"
                d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"
              />
            </svg>
          </Button>
        </div>
      </div>
    </Card>

    <!-- Error Alert -->
    <Alert
      v-if="errorMessage"
      variant="danger"
      title="Gagal Mengambil Data Pasien"
    >
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mt-1">
        <span>{{ errorMessage }}</span>
        <Button variant="danger" size="sm" @click="fetchPatients(1)">
          Coba Lagi
        </Button>
      </div>
    </Alert>

    <!-- Loading Skeleton Table -->
    <Card v-if="isLoading" class="p-0 overflow-hidden">
      <div class="p-4 space-y-4">
        <Skeleton variant="text" width="25%" height="1.5rem" />
        <div class="space-y-3">
          <Skeleton v-for="i in 6" :key="i" height="3.2rem" variant="rounded" />
        </div>
      </div>
    </Card>

    <!-- Empty State -->
    <Card
      v-else-if="patients.length === 0"
      class="p-12 text-center"
    >
      <EmptyState
        title="Tidak Ada Data Pasien"
        :description="
          searchQuery
            ? `Tidak ditemukan pasien yang sesuai dengan kata kunci '${searchQuery}'.`
            : 'Belum ada pasien terdaftar dalam sistem klinik.'
        "
      >
        <template #action>
          <Button
            v-if="searchQuery"
            variant="outline"
            size="sm"
            @click="searchQuery = ''; fetchPatients(1)"
          >
            Bersihkan Pencarian
          </Button>
          <Button
            v-else
            variant="primary"
            size="sm"
            @click="openAddModal"
          >
            + Daftarkan Pasien Pertama
          </Button>
        </template>
      </EmptyState>
    </Card>

    <!-- Data Table Container -->
    <Card v-else class="relative p-0 overflow-hidden border border-slate-800/80 bg-slate-900/70 backdrop-blur-sm">
      <div class="absolute -top-1 -left-1 font-mono text-[9px] text-slate-700/60 select-none pointer-events-none">+</div>
      <div class="absolute -top-1 -right-1 font-mono text-[9px] text-slate-700/60 select-none pointer-events-none">+</div>
      <div class="absolute -bottom-1 -left-1 font-mono text-[9px] text-slate-700/60 select-none pointer-events-none">+</div>
      <div class="absolute -bottom-1 -right-1 font-mono text-[9px] text-slate-700/60 select-none pointer-events-none">+</div>

      <div class="overflow-x-auto">
        <table class="w-full text-left text-xs text-slate-300">
          <thead class="bg-slate-950/80 text-slate-400 font-semibold uppercase tracking-wider text-2xs border-b border-slate-800">
            <tr>
              <th scope="col" class="py-3.5 px-4 sm:px-6">Nama & NIK</th>
              <th scope="col" class="py-3.5 px-4">Tgl Lahir / Usia</th>
              <th scope="col" class="py-3.5 px-4">Gender</th>
              <th scope="col" class="py-3.5 px-4">No. Telepon</th>
              <th scope="col" class="py-3.5 px-4">Status</th>
              <th scope="col" class="py-3.5 px-4">Terdaftar</th>
              <th scope="col" class="py-3.5 px-4 text-right sm:pr-6">Aksi</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-slate-800/60 bg-slate-900/40">
            <tr
              v-for="patient in patients"
              :key="patient.id"
              class="hover:bg-slate-800/30 transition-colors group"
            >
              <!-- Nama & NIK -->
              <td class="py-3.5 px-4 sm:px-6">
                <div class="font-medium text-slate-100 text-sm">
                  {{ patient.fullName }}
                </div>
                <div class="text-[11px] font-mono text-slate-400 mt-0.5">
                  {{ patient.nik || "NIK: -" }}
                </div>
              </td>

              <!-- Tgl Lahir / Usia -->
              <td class="py-3.5 px-4">
                <div class="text-slate-200">{{ formatDate(patient.dateOfBirth) }}</div>
                <div class="text-[11px] text-slate-500 mt-0.5">
                  {{ calculateAge(patient.dateOfBirth) }} tahun
                </div>
              </td>

              <!-- Gender -->
              <td class="py-3.5 px-4">
                <Badge
                  :variant="patient.gender === 'MALE' ? 'info' : 'primary'"
                  size="sm"
                  class="font-normal"
                >
                  {{ patient.gender === "MALE" ? "Laki-laki" : "Perempuan" }}
                </Badge>
              </td>

              <!-- No. Telepon -->
              <td class="py-3.5 px-4 font-mono text-slate-300">
                {{ patient.phone || "-" }}
              </td>

              <!-- Status -->
              <td class="py-3.5 px-4">
                <Badge
                  :variant="patient.isActive ? 'success' : 'default'"
                  size="sm"
                  :dot="true"
                >
                  {{ patient.isActive ? "Aktif" : "Non-aktif" }}
                </Badge>
              </td>

              <!-- Terdaftar -->
              <td class="py-3.5 px-4 text-slate-400">
                {{ formatDate(patient.createdAt) }}
              </td>

              <!-- Aksi -->
              <td class="py-3.5 px-4 text-right sm:pr-6">
                <div class="flex items-center justify-end gap-1.5">
                  <!-- Tombol Edit -->
                  <Button
                    variant="ghost"
                    size="sm"
                    @click="openEditModal(patient)"
                    class="h-8 px-2.5 text-slate-400 hover:text-blue-400 hover:bg-blue-950/40"
                    title="Ubah data pasien"
                  >
                    <svg
                      class="h-3.5 w-3.5 mr-1"
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                      stroke-width="2"
                    >
                      <path
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125"
                      />
                    </svg>
                    Edit
                  </Button>

                  <!-- Tombol Nonaktifkan (Soft Delete) -->
                  <Button
                    v-if="patient.isActive"
                    variant="ghost"
                    size="sm"
                    @click="openDeactivateModal(patient)"
                    class="h-8 px-2.5 text-slate-400 hover:text-rose-400 hover:bg-rose-950/40"
                    title="Nonaktifkan pasien"
                  >
                    <svg
                      class="h-3.5 w-3.5 mr-1"
                      xmlns="http://www.w3.org/2000/svg"
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                      stroke-width="2"
                    >
                      <path
                        stroke-linecap="round"
                        stroke-linejoin="round"
                        d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636"
                      />
                    </svg>
                    Nonaktifkan
                  </Button>
                </div>
              </td>
            </tr>
          </tbody>
        </table>
      </div>

      <!-- Pagination Toolbar -->
      <div
        class="flex flex-col sm:flex-row sm:items-center sm:justify-between px-4 sm:px-6 py-3.5 border-t border-slate-800 bg-slate-950/40 gap-3"
      >
        <div class="text-xs text-slate-400">
          Menampilkan
          <span class="font-semibold text-slate-200">
            {{ (pagination.page - 1) * pagination.limit + 1 }}
          </span>
          -
          <span class="font-semibold text-slate-200">
            {{ Math.min(pagination.page * pagination.limit, pagination.total) }}
          </span>
          dari
          <span class="font-semibold text-slate-200">{{ pagination.total }}</span>
          pasien
        </div>

        <div class="flex items-center gap-2">
          <Button
            variant="outline"
            size="sm"
            :disabled="!pagination.hasPrevPage || isLoading"
            @click="fetchPatients(pagination.page - 1)"
          >
            Sebelumnya
          </Button>

          <span class="text-xs font-medium text-slate-400 px-2">
            Hal. {{ pagination.page }} dari {{ pagination.totalPages }}
          </span>

          <Button
            variant="outline"
            size="sm"
            :disabled="!pagination.hasNextPage || isLoading"
            @click="fetchPatients(pagination.page + 1)"
          >
            Berikutnya
          </Button>
        </div>
      </div>
    </Card>

    <!-- Modal Form Tambah / Edit Pasien -->
    <Transition
      enter-active-class="transition duration-200 ease-out"
      enter-from-class="opacity-0 scale-95"
      enter-to-class="opacity-100 scale-100"
      leave-active-class="transition duration-150 ease-in"
      leave-from-class="opacity-100 scale-100"
      leave-to-class="opacity-0 scale-95"
    >
      <div
        v-if="isFormModalOpen"
        class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm overflow-y-auto"
        @click.self="closeFormModal"
      >
        <div class="relative w-full max-w-lg bg-slate-900 border border-slate-800 rounded-xl shadow-2xl p-6 my-8">
          <div class="absolute -top-1 -left-1 font-mono text-[9px] text-blue-500/40 select-none pointer-events-none">+</div>
          <div class="absolute -top-1 -right-1 font-mono text-[9px] text-blue-500/40 select-none pointer-events-none">+</div>
          <div class="absolute -bottom-1 -left-1 font-mono text-[9px] text-blue-500/40 select-none pointer-events-none">+</div>
          <div class="absolute -bottom-1 -right-1 font-mono text-[9px] text-blue-500/40 select-none pointer-events-none">+</div>

          <!-- Modal Header -->
          <div class="flex items-center justify-between pb-4 border-b border-slate-800">
            <div>
              <h3 class="text-base font-semibold text-slate-100">
                {{ isEditing ? "Perbarui Data Pasien" : "Daftarkan Pasien Baru" }}
              </h3>
              <p class="text-xs text-slate-400 mt-0.5">
                Pastikan identitas kependudukan dan nomor kontak diisi dengan benar.
              </p>
            </div>
            <button
              @click="closeFormModal"
              class="text-slate-400 hover:text-slate-200 p-1.5 rounded-lg hover:bg-slate-800 transition-colors"
            >
              <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
              </svg>
            </button>
          </div>

          <!-- Alert Error Form -->
          <Alert v-if="formError" variant="danger" class="mt-4">
            {{ formError }}
          </Alert>

          <!-- Form Body -->
          <form @submit.prevent="handleSubmitForm" class="space-y-4 mt-5">
            <!-- Nama Lengkap -->
            <div>
              <label class="block text-xs font-medium text-slate-300 mb-1">
                Nama Lengkap Pasien <span class="text-rose-400">*</span>
              </label>
              <input
                v-model="formData.fullName"
                type="text"
                placeholder="Contoh: Budi Santoso"
                class="w-full px-3 py-2 bg-slate-950 border rounded-lg text-xs text-slate-100 placeholder-slate-500 focus:outline-none transition-colors"
                :class="formFieldErrors['fullName'] ? 'border-rose-500 focus:border-rose-500' : 'border-slate-800 focus:border-blue-500'"
              />
              <p v-if="formFieldErrors['fullName']" class="text-2xs text-rose-400 mt-1">
                {{ formFieldErrors['fullName'] }}
              </p>
            </div>

            <!-- NIK -->
            <div>
              <label class="block text-xs font-medium text-slate-300 mb-1">
                Nomor Induk Kependudukan (NIK)
              </label>
              <input
                v-model="formData.nik"
                type="text"
                maxlength="16"
                placeholder="16 digit angka KTP (opsional)"
                class="w-full px-3 py-2 bg-slate-950 border rounded-lg text-xs font-mono text-slate-100 placeholder-slate-500 focus:outline-none transition-colors"
                :class="formFieldErrors['nik'] ? 'border-rose-500 focus:border-rose-500' : 'border-slate-800 focus:border-blue-500'"
              />
              <p v-if="formFieldErrors['nik']" class="text-2xs text-rose-400 mt-1">
                {{ formFieldErrors['nik'] }}
              </p>
            </div>

            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <!-- Tanggal Lahir -->
              <div>
                <label class="block text-xs font-medium text-slate-300 mb-1">
                  Tanggal Lahir <span class="text-rose-400">*</span>
                </label>
                <input
                  v-model="formData.dateOfBirth"
                  type="date"
                  class="w-full px-3 py-2 bg-slate-950 border rounded-lg text-xs text-slate-100 focus:outline-none transition-colors"
                  :class="formFieldErrors['dateOfBirth'] ? 'border-rose-500 focus:border-rose-500' : 'border-slate-800 focus:border-blue-500'"
                />
                <p v-if="formFieldErrors['dateOfBirth']" class="text-2xs text-rose-400 mt-1">
                  {{ formFieldErrors['dateOfBirth'] }}
                </p>
              </div>

              <!-- Jenis Kelamin -->
              <div>
                <label class="block text-xs font-medium text-slate-300 mb-1">
                  Jenis Kelamin <span class="text-rose-400">*</span>
                </label>
                <select
                  v-model="formData.gender"
                  class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-xs text-slate-100 focus:outline-none focus:border-blue-500 transition-colors cursor-pointer"
                >
                  <option value="MALE">Laki-laki</option>
                  <option value="FEMALE">Perempuan</option>
                </select>
              </div>
            </div>

            <!-- Nomor Telepon -->
            <div>
              <label class="block text-xs font-medium text-slate-300 mb-1">
                Nomor Telepon / WhatsApp
              </label>
              <input
                v-model="formData.phone"
                type="tel"
                placeholder="Contoh: 081234567890"
                class="w-full px-3 py-2 bg-slate-950 border rounded-lg text-xs font-mono text-slate-100 placeholder-slate-500 focus:outline-none transition-colors"
                :class="formFieldErrors['phone'] ? 'border-rose-500 focus:border-rose-500' : 'border-slate-800 focus:border-blue-500'"
              />
              <p v-if="formFieldErrors['phone']" class="text-2xs text-rose-400 mt-1">
                {{ formFieldErrors['phone'] }}
              </p>
            </div>

            <!-- Status Aktif (Hanya saat edit atau opsional) -->
            <div class="pt-2">
              <label class="flex items-center gap-2.5 cursor-pointer">
                <input
                  v-model="formData.isActive"
                  type="checkbox"
                  class="rounded bg-slate-950 border-slate-800 text-blue-600 focus:ring-blue-500/40 h-4 w-4"
                />
                <span class="text-xs font-medium text-slate-300">
                  Status Rekam Medis Aktif
                </span>
              </label>
              <p class="text-2xs text-slate-500 ml-6 mt-0.5">
                Pasien aktif dapat didaftarkan pada antrean klinik dan menerima resep obat.
              </p>
            </div>

            <!-- Modal Footer -->
            <div class="flex items-center justify-end gap-3 pt-5 border-t border-slate-800">
              <Button
                type="button"
                variant="outline"
                size="sm"
                :disabled="isSubmitting"
                @click="closeFormModal"
              >
                Batal
              </Button>

              <Button
                type="submit"
                variant="primary"
                size="sm"
                :disabled="isSubmitting"
              >
                <span v-if="isSubmitting" class="flex items-center gap-1.5">
                  <svg class="animate-spin h-3.5 w-3.5 text-white" viewBox="0 0 24 24">
                    <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4" fill="none" />
                    <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z" />
                  </svg>
                  Menyimpan...
                </span>
                <span v-else>
                  {{ isEditing ? "Simpan Perubahan" : "Daftarkan Pasien" }}
                </span>
              </Button>
            </div>
          </form>
        </div>
      </div>
    </Transition>

    <!-- Modal Konfirmasi Nonaktifkan Pasien -->
    <Transition
      enter-active-class="transition duration-200 ease-out"
      enter-from-class="opacity-0 scale-95"
      enter-to-class="opacity-100 scale-100"
      leave-active-class="transition duration-150 ease-in"
      leave-from-class="opacity-100 scale-100"
      leave-to-class="opacity-0 scale-95"
    >
      <div
        v-if="isDeactivateModalOpen"
        class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm"
        @click.self="closeDeactivateModal"
      >
        <div class="relative w-full max-w-md bg-slate-900 border border-slate-800 rounded-xl shadow-2xl p-6">
          <div class="absolute -top-1 -left-1 font-mono text-[9px] text-rose-500/40 select-none pointer-events-none">+</div>
          <div class="absolute -top-1 -right-1 font-mono text-[9px] text-rose-500/40 select-none pointer-events-none">+</div>
          <div class="absolute -bottom-1 -left-1 font-mono text-[9px] text-rose-500/40 select-none pointer-events-none">+</div>
          <div class="absolute -bottom-1 -right-1 font-mono text-[9px] text-rose-500/40 select-none pointer-events-none">+</div>

          <div class="flex items-center gap-3 text-rose-400 mb-3">
            <div class="p-2 rounded-lg bg-rose-950/50 border border-rose-800/80">
              <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
              </svg>
            </div>
            <h3 class="text-base font-semibold text-slate-100">
              Nonaktifkan Pasien?
            </h3>
          </div>

          <p class="text-xs text-slate-300 leading-relaxed mb-4">
            Apakah Anda yakin ingin menonaktifkan pasien
            <span class="font-bold text-slate-100">
              {{ patientToDeactivate?.fullName }}
            </span>?
            Data riwayat kunjungan dan resep tetap tersimpan utuh demi integritas rekam medis.
          </p>

          <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-800">
            <Button
              variant="outline"
              size="sm"
              :disabled="isDeactivating"
              @click="closeDeactivateModal"
            >
              Batal
            </Button>
            <Button
              variant="danger"
              size="sm"
              :disabled="isDeactivating"
              @click="handleConfirmDeactivate"
            >
              <span v-if="isDeactivating">Menonaktifkan...</span>
              <span v-else>Ya, Nonaktifkan</span>
            </Button>
          </div>
        </div>
      </div>
    </Transition>
  </div>
</template>
